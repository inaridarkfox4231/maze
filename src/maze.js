...
// はい
// 完成ではないです。動くようにしただけ。モチベ上がんないからこれ以上いじらないかもしれないけど。

// 思ったこと1:
// プレイヤーの位置をユニフォームで渡して、近くのタイルしか描画されないようにするといいかも。
// とりあえずグリッドで5x5の外にあったら描画しないようにしてみる。
// 画面外のものを描画しても仕方ないので。
// 思ったこと2:
// たとえば50ごとにチェックポイントを用意するとして、そういうタイルの種類をひとつだけ用意したうえで、
// プレイヤーのスタートからの進捗を計算して渡して、それに応じてタイルの柄を操作して（つまりテクスチャの種類を）
// 50とか100とか表示されるようにすればあらかじめ50とか100とか書いておかないで済むよね
// 思ったこと3:
// トーラスステージ作れるよ。反対側行くときに、0.5まで進んだところでワープさせればいい。向こう側が見えないように
// そこにはなんかきらきらのもやもやみたいな・・大神の鏡みたいなの用意すればいい。で、くぐると反対側、という。

// 3Dやめまーーーーす。
// 2Dで開発しましょう。
// で、プレイヤーの移動に関しては今の仕様でいい。direction関連についてだけど、
// もう上下左右に限ることにしたので、0,1,2,3でやっちゃおう。それでどこまで簡単になるか知らんけど。
// まあ・・あー、やめとこうか、うん。んー。
// ごめんやっぱ一般で作ってみるわ。ななめとかにするかも知らんし。
// うん、うん。3Dは爆死したんで。凍結！

// 今考えてること
// マウスの位置に従って方向が矢印で表現される、マウスダウンで進行。進み方はこっちと一緒なのでうまく移植してね。
// んでプレイヤーがマウスに近いほど速度が上がる仕様にする感じで。もしくはマウスダウンの間だけ加速する、でも可。
// 次にサイズは640x480でここに落とす。グリッドサイズは32x32で100x100の場合3200x3200とかなるわね。
// 3200x3200とか作りたい・・・
// キャンバスは640x640にするけどね。160のところで、なんかする。
// オフセットはプレイヤーが中心に来るようにする。
// プレイヤーは攻撃ができる。といっても直進弾とかそういうのだけ。ホーミングとかも実装するかは未定。
// でね、敵が徘徊する。これが自機狙いとか撃ってくる。それをうまくよけつつゴールに到着。
// カギと鍵穴とかもあったりする
// 敵の挙動は基本直進で行き止まりの時だけ引き返す。あとアイテムや鍵穴などは通れない、とみなす。
// プレイヤーも通れない・・接触してエンターで取得、みたいな？わかんないけど。
// 迷路の形について。
// 六角形とか、円弧が連なるのとかあったら面白そうね。
// まあいいや、基本線分で結んでください、あんま複雑なのはやめて。
// というより、そういうのよりあれ、2階建てやりたいのよ。2階建て。4階建てとか。
// 100x100とかもいいけどそういうのも面白そう。マスにはまると自動的に別の階に行くの。
// トランジションは30フレームくらいで素早くね。遅いとイライラするから。
// 現時点では以上。
// あとトーラス作りたいわね・・・・
// 複数階建てで4階の上が1階とかそういうのも面白そう・・・
// 2階の外に出たら3階につながるとか

// 80x80でも一瞬で描画終わるんだけどね。うんとね、
// クリアする気しない・・・でかすぎて（（（
// 工夫はします。たとえばvalueに応じて床の色を変える。つまりhueの値を0～80まで大きく。
// つまりvalueのMAXを取ってMAXで割って80を掛けてhueの値としてそれで床の色を決める感じね。
// ゴールに近づくほど色が紫に近づく感じで。それでなんとなく察せられる。
// あとはまあ10で割って間の1～9にそれっぽくなんかおいてそれでわかりやすいようにする、近づいてることが。
// 80x80なのに20x15しか表示されないのクソゲー感すごいけどやってみたい（（
// ボードはGLSLで描画するか。一度描画したらそれっきりであってもその方が一瞬で終わるし。うん。
// タイルはポイントスプライトで、色合いをhueで指定して・・
// RGB→HSBしてHSBでhueをいじってHSB→RGBで戻すとか。あるいはテクスチャで・・あー、めんどい。テクスチャにそのまま掛け算だ。それで。
// 黒～白のコントラストで。
// それもばかばかしいからもういっそポイントスプライトのPointCoordで正方形作っちゃえ(32x32)。色合いをzにいれとく（valueから計算する）
// レイヤーのz座標はキャンバス指定に使う。あくまで2Dだから問題ない。で、valueから0.0～0.8のhue割り出してぶちこむ。
// 色使っちゃうからスタートとゴールは別途考える・・そうだ、スタートは白に黒字でS,ゴールは黒に白字でGにしよう。OK!
// 線についてもたくさん引くことになるので・・黒い線をね。んー。
// スタートは赤い縁取りでゴールは紫の・・んー。
// まあいいや、satを0.5にして・・うん。スタートとゴールはテクスチャで。
// zのところの整数部分でいつものように。0は床、1とか2のときに別のテクスチャ使う感じでいいよ。それで。
// んで、上と下の階をつなぐワープとかそういうのとかそういうの。

// strokeのと床とそれぞれ用意して重ね合わせればOK.
// トーラスは上と下がつながってる場合は辺を描画しない感じで。
// 上の階と下の階をつなぐ場合はイベント処理で。
// 複数階の場合、3次元のベクトルを用いる。3つ目の座標は階層をあらわす。0とか1とか2とか。
// それらをつなぐ辺は描画には使わない。マップの描画は階層ごとに行う感じで、階層ごとに床としきいを描画する。
// 敵はワープを使わない（階層をまたがない）。ただトーラス状の通路は移動する。その場合当たり判定とかは
// 中心がどっちにあるかで決まる感じ、たとえば体1/4しか出てないなら攻撃を受けないなど。こちらも同様。
// そこらへんは個別の処理が必要になるでしょうね・・大変。
// つまり通常のedgeの他に階層をつなぐedgeやトーラスedgeが要る。
// 階層つなぎedgeは0と1が一瞬で切り替わる感じ。
// トーラスedgeは0.5も閾値の役割を果たす。0.5より大きくなったり小さくなったりする度に（例えば0.5より小さかったが以上になるとか）
// んー
// 0.5より小さい間は0からdirection方向に測り
// 0.5以上の場合は1からdirectionと逆方向に測ればいいわね
// それで勝手にワープしてくれる
// それでやろう
// トーラスでもいいけど
// 左右をクロスさせてもいいわね
// 上側で0,1,2,...,w-1を下側のw-1,...,2,1,0につなぐとか（方向は同じで）。左右もそうするとか。
// もっと面白くなりそう・・・・
// 3Dクソだ。2Dばんざいだ。

// また変更。階層つなぎやめる。はしっこ接続オンリーにする。
// それで違うフロアに行くようなら違うフロアであることをどこかに明示などする。
// たとえばステージ11なら11-1とか11-2とかどっかに表示しておいて違うフロアであることをわかりやすく・・
// だってそうしないといろいろめんどくさいんだよ・・
// で、移動もシームレスに行う感じね。
// ただしつなぎ方に制限をもうける。常に上に行くなら下から、下に行くなら上から、など。
// ずれるつなぎ方をするとしてもそこだけは間違えないように注意する。OK!

// プレイヤーはとりあえず正方形でも回転させておこう。

// 方針変更。
// マウスの方向に動く。マウスが近いほど速く動く。マウスダウンの間だけ弾が出る。アイテムを取ると増えていく。ダメージ受けると減る。
// イメージ的にはコラプスみたいにパターンが増えてそれら全部出すみたいなあれ、まあSTGでよくある仕様。使い分けはしないってこと。
// STG作る際もその方針で行くね。そのためにはparallel実装しないといけないけど。

// ---------------------------------------------------------------------------------------------------------------- //

// 何からやろうか。
// まずcontourはcontoursにしていくつかの折れ線の集合にする（輪郭線が途切れている場合があるので）（たとえば上下だけつなぐなど）
// これに応じて周囲の線を引く。グラフィックは2枚用意。
// ひとつはWEBGLでこれはポイントスプライトで高速にタイル群を描画するのに使う。
// ひとつひとつのタイルをGLSLで描きたいので。pointCoordを用いたGLSL描画でささっと。
// それを背景にしてまずスタートやゴールなどの特別なタイルを2Dのimageで配置。
// その上に2Dで線をしきいとして引く。これは普通に引く。
// これで完成～
// って思ったけどやっぱしきいもスタートやゴールもWEBGLで描いちゃいたいかなぁ
// 描画は配列にぶち込まれた順なので線分であってもあとで普通に追加される
// しきいがポイントスプライト使えるなら単純な線分に限らずいろんなパターンを使える（たとえばレンガみたいなのとか）（ツタみたいなとか）
// 2Dだと線分以上のことができない（高速でって意味だけど）それは不便
// まあ床がカラフルだからなるたけ地味じゃないといけないけど（濃い茶色とか濃い緑、黒系など）
// 床の色構成を逆にする（中央色で周囲白）んで敷居が分かりやすいように。OK!

// 複数枚を想定して、頂点ベクトルは3次元とし、z座標を0,1,2,...みたいにして、グラフィックも配列みたいにする・・
// 1枚であれば0番だけ使う感じ。ゲームボードは一つだけ。プレイヤーがどの階層にいるかで使うグラフィックを選ぶ、そのあとはオフセット処理。


// 完成したこれを、ゲームボード640x480に切り取りで背景として使う。
// 640x480が最小サイズなので余計なことは考えない。
// グリッドは32x32を想定しているけど変えるかもしれないって書きたかっただけ。
// 不便だなこれ！！！（怒）
// プレイヤーの位置に応じてオフセット処理して描画する。
// （プレイヤーが左右320上下240より内側にあるとき中心にくるように）

// それとは別にプレイヤーの位置をマウスの位置に応じて動かせるようにする
// 3Dでディレクションとして処理していた値のアップデートをマウスで行う。ただし・・・
// プレイヤーの画面内における位置を（640x480に落とした場合の位置を）参照しないといけないところが気を付けるポイント
// これを持たせておき、それと画面のオフセットをもとに・・
// 今現時点では640x480をそのまま使うのでそれベースの値でOK.
// たとえば迷路が大きくてプレイヤーが中心にいるときなどはその中心をベースにマウス位置とそれとから進行方向を割り出す必要がある
// そこが注意点。

// ここまでできればとりあえずプレイヤーをスタートからゴールまで誘導できるようにはなる。
// ゲームボードに背景として迷路を描画した後でプレイヤーを描画する。毎フレーム。プレイヤーは画面内に常にいる（はしっこを通って
// 別の場所に行くとき半身が隠れる場合を除いて）。とりあえずちいさめの正方形を回しておいてください。お疲れさまでした！

// えーと、複数階層にまたがる場合は半分ずつ描画する。辺を。

// 変更
// いろいろ
// まず、描画は線分をやる。違うフロアにまたがる場合はそれぞれ半分ずつ
// 理由は結局空間をゆがませる場合にしきいだとcontourとか描画するのめんどくさいし、
// 描画したとしても見た目的に区別つかないの嫌だから。
// んで斜めの場合とかも。というか斜めの場合も許したいから線分描画するの。
// スタートとゴールはそれとは別に描画する。円形みたいなので。グリッドは線分の長さとする。だから円でOKとなる。webglのcircleもしくは
// ポイントスプライトでもいいけど
// もしかしたらイベント増やすかもしれないしそういう意味でそこはポイントスプライトの方がいいと思う。
// 大きさはグリッドより若干狭めて辺が完全に隠れないよう注意する。

// というわけで用意するのはフロア数分の枚数、線分とイベントが描画された
// あー・・
// イベントは毎フレームでもいいかな・・あの、あれ、途中でゴールが出現するとかそういう感じでもいいかなって思っちゃった
// スタートはいいとして
// 線分をstrokeShaderでやりたいのはまあ微妙なんだけどあれ、頂点ごとのhueの値でlerpみたいにしたいのよ。
// 赤からオレンジへのグラデとかさ。そういうのshaderでないと出来ないから。線分でも速いけどね。

// 全部白でいいからもうプレイヤー動くとこまで作っちゃえ
// 全部2Dでいいです
// その代わり描画をその、1フレームに3つずつくらいで中心からばーって配置されていく感じにしたらどうかなと
// 60フレームくらいでイージングかけてとんでいく感じ・・
// そういうのもとりあえずいいからとりあえずマウスでプレイヤーが迷路を探索できるようにしたいです
// んでゴールが近づいたらわかりやすくするとか周囲をシェーダで暗くしていいよあとであとであとで
